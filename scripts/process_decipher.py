...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="process the decipherment data directory")
    parser.add_argument('--input', type=str, help="this is a directory")
    args = parser.parse_args()

    save_dir = args.input

    train0 = os.path.join(args.input, "decipher.train.0")
    train1 = os.path.join(args.input, "decipher.train.1")

    dev0 = os.path.join(args.input, "decipher.dev.0")
    dev1 = os.path.join(args.input, "decipher.dev.1")

    test0 = os.path.join(args.input, "decipher.test.0")
    test1 = os.path.join(args.input, "decipher.test.1")
    ...
    with open(train0, "r") as fin:
        for line in fin:
            ftrain_t.write(line)
            ftrain_a.write("original\n")
            ftrain_a0.write("original\n")
